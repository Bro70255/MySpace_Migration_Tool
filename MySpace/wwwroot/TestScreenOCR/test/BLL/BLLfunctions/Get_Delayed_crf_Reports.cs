        public static DataTable Get_Delayed_crf_Reports(int firm, int Employee_Code, int UserType, int Team_id, int Unit)
        {
            try
            {
                DataTable dtDetail;
                using (SqlConnection NewConnection = new SqlConnection(Connection.ConnectionString))
                {
                    ...
                    using (SqlTransaction NewTransation = NewConnection.BeginTransaction())
                    {
                        try
                        {
                            dtDetail = CRF_Tracker_dal.Get_Delayed_crf_Reports(NewTransation, firm, Employee_Code, UserType, Team_id, Unit);
