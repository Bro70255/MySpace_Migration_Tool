        public static DataTable Get_Notification(int EMP_CODE, int FIRM, int UserType, int Team_id, int Unit)
        {
            try
            {
                DataTable dtDetail;
                using (SqlConnection NewConnection = new SqlConnection(Connection.ConnectionString))
                {
                    ...
                    using (SqlTransaction NewTransation = NewConnection.BeginTransaction())
                    {
                        try
                        {
                            dtDetail = CRF_Tracker_dal.Get_Notification(NewTransation, EMP_CODE, FIRM, UserType, Team_id, Unit);
