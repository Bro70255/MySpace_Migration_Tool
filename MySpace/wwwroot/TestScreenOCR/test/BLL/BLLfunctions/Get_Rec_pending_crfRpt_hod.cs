        public static DataTable Get_Rec_pending_crfRpt_hod(int firm, int Employee_Code, int UserType, int Team_id, int Unit)
        {
            try
            {
                DataTable dtDetail;
                using (SqlConnection NewConnection = new SqlConnection(Connection.ConnectionString))
                {
                    ...
                    using (SqlTransaction NewTransation = NewConnection.BeginTransaction())
                    {
                        try
                        {
                            dtDetail = CRF_Tracker_dal.Get_Rec_pending_crfRpt_hod(NewTransation, firm, Employee_Code, UserType, Team_id, Unit);
