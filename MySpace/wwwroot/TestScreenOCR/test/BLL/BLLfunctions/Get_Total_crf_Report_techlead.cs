        public static DataTable Get_Total_crf_Report_techlead(int firm, int Team_id)
        {
            try
            {
                DataTable dtDetail;
                using (SqlConnection NewConnection = new SqlConnection(Connection.ConnectionString))
                {
                    ...
                    using (SqlTransaction NewTransation = NewConnection.BeginTransaction())
                    {
                        try
                        {
                            dtDetail = CRF_Tracker_dal.Get_Total_crf_Report_techlead(NewTransation, firm, Team_id);
