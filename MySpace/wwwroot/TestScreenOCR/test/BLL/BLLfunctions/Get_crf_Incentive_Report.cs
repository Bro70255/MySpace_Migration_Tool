        public static DataTable Get_crf_Incentive_Report(DateTime Startdate, DateTime Enddate, int firm)
        {
            try
            {
                DataTable dtDetail;
                using (SqlConnection NewConnection = new SqlConnection(Connection.ConnectionString))
                {
                    ...
                    using (SqlTransaction NewTransation = NewConnection.BeginTransaction())
                    {
                        try
                        {
                            dtDetail = CRF_Tracker_dal.Get_crf_Incentive_Report(NewTransation, Startdate, Enddate, firm);
