        public static DataTable Get_developerwise_Report(DateTime Startdate, DateTime Enddate, int Developer, int firm)
        {
            try
            {
                DataTable dtDetail;
                using (SqlConnection NewConnection = new SqlConnection(Connection.ConnectionString))
                {
                    ...
                    using (SqlTransaction NewTransation = NewConnection.BeginTransaction())
                    {
                        try
                        {
                            dtDetail = CRF_Tracker_dal.Get_developerwise_Report(NewTransation, Startdate, Enddate, Developer, firm);
