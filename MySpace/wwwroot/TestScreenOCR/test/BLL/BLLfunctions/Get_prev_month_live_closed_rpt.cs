        public static DataTable Get_prev_month_live_closed_rpt(int firm, int Employee_Code, int UserType, int Team_id, int Unit)
        {
            try
            {
                DataTable dtDetail;
                using (SqlConnection NewConnection = new SqlConnection(Connection.ConnectionString))
                {
                    ...
                    using (SqlTransaction NewTransation = NewConnection.BeginTransaction())
                    {
                        try
                        {
                            dtDetail = CRF_Tracker_dal.Get_prev_month_live_closed_rpt(NewTransation, firm, Employee_Code, UserType, Team_id, Unit);
