        public static void Update_Head_reject(int HEAD_REJECT, int EMP_CODE, string crf_ID, string Remark)
        {
            try
            {
                using (SqlConnection NewConnection = new SqlConnection(Connection.ConnectionString))
                {
                    ...
                    using (SqlTransaction NewTransation = NewConnection.BeginTransaction())
                    {
                        try
                        {
                            CRF_Tracker_dal.Update_Head_reject(NewTransation, HEAD_REJECT, EMP_CODE, crf_ID, Remark);
