        public static DataTable Get_Rec_pending_crfRpt_hod(SqlTransaction newTransaction, int firm, int Employee_Code, int UserType, int Team_id, int Unit)
        {
            ...
            try
            {
                ...
                SqlHelper.FillDatatable(newTransaction, CommandType.StoredProcedure, StoreProcedure.GET_REC_PENDING_CRFRPT_HOD, dtDetails, 0, parameters);
            }
            catch (Exception ex)
            {
                throw ex;
