        public static DataTable Get_prev_month_live_closed_rpt(SqlTransaction newTransaction, int firm, int Employee_Code, int UserType, int Team_id, int Unit)
        {
            ...
            try
            {
                ...
                SqlHelper.FillDatatable(newTransaction, CommandType.StoredProcedure, StoreProcedure.GET_PREV_MONTH_LIVE_CLOSED_RPT, dtDetails, 0, parameters);
            }
            catch (Exception ex)
            {
                throw ex;
